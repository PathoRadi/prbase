...
async function sendToAdmin(username, project) {
  try {
    ...
    const mailOptions = {
      ...
      html: `<div>New Upload Images from ${username}.</div><br/><div>Upload Info: ${API_URL}/${id}</div><div> Download Link:  <a href='https://prbase.azurewebsites.net/download-images?username=${username}&project=${project}'/> https://prbase.azurewebsites.net/download-images?username=${username}&project=${project} </a></div><br/>`,
    };
...
router.post("/create", (req, res) => {
  ...
  Object.values(uploadInfo).map((info) => {
    ...
    const organ = (info.otherRrgan) ? otherRrgan :info.organ;
